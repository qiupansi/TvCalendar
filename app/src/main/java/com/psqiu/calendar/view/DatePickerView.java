...
public class DatePickerView extends View {
    ...
    private MyTimerTask mUpdateViewTask;
    ...
    private Handler mUpdateViewHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (Math.abs(mMoveLen) < SPEED) {
                mMoveLen = 0;
                if (mUpdateViewTask != null) {
                    mUpdateViewTask.cancel();
                    mUpdateViewTask = null;
                    performSelect();
                }
            } else {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        int seledIndex = -1;
        if (keyCode == KeyEvent.KEYCODE_DPAD_UP) {              // press the up key

            isKeyPressed = true;
            seledIndex = mCurrentSelected - 1;
        } else if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN) {     // press the down key

            isKeyPressed = true;
            seledIndex = mCurrentSelected + 1;
        }

        if (seledIndex >= mDataList.size() || seledIndex < 0) { // error index
            return super.onKeyDown(keyCode, event);
        }

        if (isKeyPressed) {                                     // update data

            initSeledParams(seledIndex);
            updateViewTask();
            return true;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mMoveLen > MARGIN_ALPHA * mMinTextSize / 2) {               // 向上
                ...
                } else if (mMoveLen < -MARGIN_ALPHA * mMinTextSize / 2) {       // 向下
    ...
    private void updateViewTask() {
        if (mUpdateViewTask != null) {
            mUpdateViewTask.cancel();
            mUpdateViewTask = null;
        }

        mUpdateViewTask = new MyTimerTask(mUpdateViewHandler);
        mUpdateViewTimer.schedule(mUpdateViewTask, 0, 10);
    }
    ...
    public void setSelected(int selected) {
        if (selected >= mDataList.size() | selected < 0) {
            return;
        }

        initSeledParams(selected);
        invalidate();
    }

    private void initSeledParams(int selected) {
